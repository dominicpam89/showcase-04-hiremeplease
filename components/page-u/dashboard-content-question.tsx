...
export default async function Question({ question }: Props) {
	return (
		<Card
			key={question.id}
		...
		>
			<CardHeader aria-label="question-card-header" className="space-y-1">
				<CardTitle
				...
				<CardDescription aria-label="question-card-description">
					<CardDescriptionUser uid={question.uid} />
				</CardDescription>
				<DashboardContentTags tags={question.tags} />
			</CardHeader>
			<CardContent aria-label="question-card-content" className="pb-0">
			...
			>
				<Button asChild variant="link">
					<Link href={`/u/questions/${question.id}`} className="text-sm">
						Show More
					</Link>
				</Button>
			</CardFooter>
