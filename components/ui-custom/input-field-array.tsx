...
export default function InputFieldArray<T extends FieldValues>({
	placeholder,
	icon,
	name,
	label,
	...
	const userInputOnChange = (e: ChangeEvent<HTMLInputElement>) => {
	...
	useEffect(() => {
		if (userInput.includes(",")) {
			...
			setTags((ps) => Array.from(new Set([...ps, val])));
			setUserInput("");
		}
	}, [userInput]);
	...
	return (
		<div aria-label="input-field" className="flex flex-col gap-2 w-full">
			{label && (
				<Label htmlFor="user-input" className="text-gray-500">
					{label}
				</Label>
			)}
			<div className="w-full flex gap-2 items-center">
				...
				>
					<TagsUI tags={tags} mutateTags={mutateTags} />
					<input
						id={name}
...
function TagsUI({ tags, mutateTags }: TagsProps) {
	const tagsWithId = tags.map((val) => ({ id: generateRandomId(), val }));
			...
			{tagsWithId.map((tag) => {
				return (
					<Badge
						key={tag.id}
						variant="secondary"
						onClick={removeTag.bind(null, tag.id)}
					>
						{tag.val}
					</Badge>
				);
