...
export default function NavlistDesktop() {
	...
	const toggleMinimizeSidebar = () => setMinimizedSidebar(!minimizedSidebar);
	return (
		<nav
				...
				>
					...
					<div aria-label="header-content">
						<p className="text-xs opacity-50 text-justify text-pretty break-all">
							Welcome to TeaTac (Teach and Tackle), ask the real
							questions to teachers for knowledge bombs
						</p>
					</div>
				</div>
			)}
			{minimizedSidebar && <LogoUIWithLink size="lg" />}
			<ul
...
function MinimizeSidebarToggle({ toggle, minimized }: ToggleProps) {
	return (
		<div
